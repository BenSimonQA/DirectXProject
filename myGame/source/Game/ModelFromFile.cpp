...
namespace Rendering
{
    ...
    ModelFromFile::ModelFromFile(Game& game, Camera& camera, const std::string modelFilename, const std:: wstring texturize)
          ...
          mInputLayout(nullptr), mWorldMatrix(MatrixHelper::Identity), mVertexBuffer(nullptr), mIndexBuffer(nullptr), mIndexCount(0), modelFile(modelFilename)
    {
		...
		mModelValue = 0;
		textureName = texturize;
    }

	ModelFromFile::ModelFromFile(Game& game, Camera& camera, const std::string modelFilename, const std::wstring ModelDes, int ModelValue, const std::wstring texturize)
		: DrawableGameComponent(game, camera),
		mEffect(nullptr), mTechnique(nullptr), mPass(nullptr), mWvpVariable(nullptr), mTextureShaderResourceView(nullptr), mColorTextureVariable(nullptr),
